...
namespace hsd
{
	namespace unicode
	{
		template <typename CharT> requires (sizeof(CharT) == 1)
		static constexpr usize length(const CharT* str)
		{
			usize _len = 0;

			while (str[_len] != static_cast<CharT>('\0'))
			{
				// Check if there is a character composed of four UTF-8 bytes.
				if (
					(static_cast<add_unsigned_t<CharT>>(str[_len]) 	   & 0xF8) == 0xF0 && 
					(static_cast<add_unsigned_t<CharT>>(str[_len + 1]) & 0xC0) == 0x80 && 
					(static_cast<add_unsigned_t<CharT>>(str[_len + 2]) & 0xC0) == 0x80 && 
					(static_cast<add_unsigned_t<CharT>>(str[_len + 3]) & 0xC0) == 0x80
				) {
					_len += 4;
				}
				else if (
					(static_cast<add_unsigned_t<CharT>>(str[_len]) 	   & 0xE0) == 0xC0 &&
					(static_cast<add_unsigned_t<CharT>>(str[_len + 1]) & 0xC0) == 0x80
				) {
					_len += 2;
				}
		...
		template <typename CharT> requires (sizeof(CharT) == 2)
		static constexpr usize length(const CharT* str)
		{
			usize _len = 0;
			...
			while (str[_len] != static_cast<CharT>('\0'))
			{
				// Check if there is a character composed of two UTF-16 bytes.
				if (
					(static_cast<add_unsigned_t<CharT>>(str[_len]) 	   & 0xFC00) == 0xD800 && 
    	            (static_cast<add_unsigned_t<CharT>>(str[_len + 1]) & 0xFC00) == 0xDC00
				) {
					_len += 2;
				}
				else
		...
		template <typename CharT> requires (sizeof(CharT) == 4)
		static constexpr usize length(const CharT* str)
		{
			usize _len = 0;
