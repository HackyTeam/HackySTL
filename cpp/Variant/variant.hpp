...
struct _variant_tag_first_member {};
struct _variant_tag_rest_member {};

template <typename _Stor>
struct _variant_storage_traits;
...
struct _variant_storage<_Tfirst, _Trest...> {
    ...
    using _RestTraits = _variant_storage_traits<_variant_storage<_Trest...>>;

    // Default constructor
    constexpr _variant_storage() : _value_first() {}

    // Copy constructors
    constexpr _variant_storage(_variant_tag_first_member, const _variant_storage& o)
        : _value_first(o._value_first) {}
    constexpr _variant_storage(_variant_tag_rest_member, const _variant_storage& o, size_t id)
        : _storage_rest(o._storage_rest, id) {}
    constexpr _variant_storage(const _variant_storage& o, size_t id)
        : _variant_storage((id == 0)
            ? _variant_storage(_variant_tag_first_member(), o)
            : _variant_storage(_variant_tag_rest_member(), o, id-1)) {}

    // Move constructors
    constexpr _variant_storage(_variant_tag_first_member, _variant_storage&& o)
        : _value_first(move(o._value_first)) {}
    constexpr _variant_storage(_variant_tag_rest_member, _variant_storage&& o, size_t id)
        : _storage_rest(move(o._storage_rest), id) {}
    constexpr _variant_storage(_variant_storage&& o, size_t id)
        : _variant_storage((id == 0)
            ? _variant_storage(_variant_tag_first_member(), move(o))
            : _variant_storage(_variant_tag_rest_member(), move(o), id-1)) {}
    
    // Forwarding constructors
    template <typename _Ty>
    constexpr _variant_storage(_variant_tag_first_member, _Ty&& val)
        : _value_first(forward<_Ty>(val)) {}
    template <typename _Ty, size_t _Idx>
    constexpr _variant_storage(_variant_tag_rest_member, _Ty&& val, std::integral_constant<size_t, _Idx> id)
    ...
    constexpr static _variant_storage _construct_index(_Ty&& val, std::integral_constant<size_t, _Idx>) {
        if constexpr (_Idx == 0)
            return _variant_storage(_variant_tag_first_member(), forward<_Ty>(val));
        else
            return _variant_storage(_variant_tag_rest_member(), forward<_Ty>(val), std::integral_constant<size_t, _Idx-1>());
    }
};


template <typename... _Ts>
struct _variant_storage_traits<_variant_storage<_Ts...>> {
...
template <typename _Tfirst, typename... _Trest>
struct _variant_storage_traits<_variant_storage<_Tfirst, _Trest...>> {
...
private:
    void _destroy() {
        _StorageTraits::destroy(_Value, _StoredIndex);
    }
    ...
    template <typename _Tofirst, typename... _Torest>
    struct _select_overload_for<_Tofirst, _Torest...> : _select_overload_for<_Torest...> {
        using _select_overload_for<_Torest...>::get;
        constexpr static _Tofirst get(_Tofirst);
    };
    ...
    using _StorageTraits = _variant_storage_traits<_variant_storage<_Tfirst, _Trest...>>;
public:
